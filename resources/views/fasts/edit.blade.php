<x-app-layout>
    <x-slot name="header">
        <div style="display:flex;align-items:center;justify-content:space-between">
            <div>
                <h2 class="app-title">Modifier un jeûne</h2>
                <p style="color:var(--text-muted);font-size:13px;margin-top:4px">
                    {{ \Carbon\Carbon::parse($fast->fast_date)->format('d/m/Y') }}
                </p>
            </div>
    ...
    <div class="page-content fade-in" style="max-width:680px">
        <div class="app-card app-card-gold" style="padding:2rem">
            ...
            <form method="POST" action="{{ route('fasts.update', $fast) }}">
                @csrf
                @method('PUT')

                @php $selectedLeaders = $fast->leaders->pluck('id')->toArray(); @endphp
                ...
                <div style="margin-bottom:1.5rem">
                    ...
                    <input type="date" name="fast_date"
                        value="{{ old('fast_date', $fast->fast_date) }}"
                ...
                <div style="margin-bottom:1.5rem">
                    ...
                    <select name="leader_ids[]" multiple
                        style="width:100%;background:#0D1333;border:1px solid rgba(99,132,255,0.15);border-radius:10px;color:#E8EDF8;font-size:14px;padding:6px;height:160px;outline:none;transition:border-color 0.2s"
                        onfocus="this.style.borderColor='#C9A84C'" onblur="this.style.borderColor='rgba(99,132,255,0.15)'">
                        @foreach($leaders as $leader)
                            <option value="{{ $leader->id }}"
                                style="padding:8px 10px;background:#0D1333;color:#E8EDF8"
                                {{ in_array($leader->id, old('leader_ids', $selectedLeaders)) ? 'selected' : '' }}>
                                {{ $leader->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div style="margin-bottom:1.5rem">
                    ...
                    <select name="fast_type" required
                        style="width:100%;background:#0D1333;border:1px solid rgba(99,132,255,0.15);border-radius:10px;color:#E8EDF8;font-size:14px;padding:10px 14px;outline:none;transition:border-color 0.2s;appearance:auto"
                        onfocus="this.style.borderColor='#C9A84C'" onblur="this.style.borderColor='rgba(99,132,255,0.15)'">
                        @foreach($fastTypes as $value => $label)
                            <option value="{{ $value }}" style="background:#0D1333;color:#E8EDF8"
                                {{ old('fast_type', $fast->fast_type) === $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div style="margin-bottom:2rem">
                    ...
                    <input type="number" name="prayer_minutes" min="0" max="1440" placeholder="Ex: 30"
                        style="width:100%;background:#0D1333;border:1px solid rgba(99,132,255,0.15);border-radius:10px;color:#E8EDF8;font-size:14px;padding:10px 14px;outline:none;transition:border-color 0.2s"
                        onfocus="this.style.borderColor='#C9A84C'" onblur="this.style.borderColor='rgba(99,132,255,0.15)'"
                        value="{{ old('prayer_minutes', $fast->prayer_minutes) }}">
